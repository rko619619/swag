feature 'Sign in' do
  ...
  it 'user is able to log in with valid email and password' do
    login_page.login(VALID_NAME, VALID_PASSWORD)
    expect(current_url).to eq PRODUCTS_PAGE
  end

  it 'user is not able to log in with invalid email and password' do
    login_page.login(INVALID_NAME, INVALID_PASSWORD)
    expect(login_page.get_error_message_on_login).to eq ERROR
  end

  it 'user is not able to log in with empty email and empty password' do
    login_page.login('', '')
    expect(login_page.get_error_message_on_login).to eq ERROR_USERNAME
  end

  it 'user is not able to log in with empty username and not empty password' do
    login_page.login('', 'INVALID_PASSWORD')
    expect(login_page.get_error_message_on_login).to eq ERROR_USERNAME
  end

  it 'user is not able to log in with not empty username and empty password' do
    login_page.login(INVALID_NAME, '')
    expect(login_page.get_error_message_on_login).to eq ERROR_PASSWORD
  end

  it 'locked out user is not able to log in with valid username and password' do
  ...
  it 'user is not allowed to visit other pages without autorization' do
    visit(PRODUCTS_PAGE)
